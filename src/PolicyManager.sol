...
contract PolicyManager is IPolicyManager, Ownable {
    ...
    mapping(uint256 => IOracleCommittee) public policyOracleCommittee;
    ...
    // policyId => assetSymbolInBytes32
    mapping(uint256 => bytes32) public policyAssetSymbolBytes32;
    ...
    // Below is a hack so we can demo
    uint256[] allPolicies;
    ...
    modifier requireOracleCommitteeSet(uint256 _policyId) {
        require(address(policyOracleCommittee[_policyId]) != address(0), "Oracle committee not set");
        _;
    }

    modifier onlyOwnerOrOracleCommittee(uint256 _policyId) {
        require(
            msg.sender == owner() || msg.sender == address(policyOracleCommittee[_policyId]),
            "Only system address or owner can call this function"
        );
        _;
    }
    ...
    function createPolicy(
        ...
        console.log("Done with starter");
        //      policyOracleCommittee[policyCounter] = new OracleCommittee(
        //     policyCounter,
        //     address(this),, currencyInsured, blockNumber,
        ...
        policyAssetSymbolBytes32[policyCounter] = _stringToBytes32(currencyInsuredSymbol);
        policyCollateral[policyCounter] = currencyInsurer;
        allPolicies.push(policyCounter);
        policyCounter++;
    ...
    function depegEndPolicy(uint256 _policyId)
        public
        requireOracleCommitteeSet(_policyId)
    ...
    function activatePolicy(uint256 _policyId) public requireOracleCommitteeSet(_policyId) onlyOwner {
    ...
    function setOracleCommittee(uint256 _policyId, address _oracleCommittee) public onlyOwner {
        require(address(policyOracleCommittee[_policyId]) == address(0), "Oracle committee already set");
        policyOracleCommittee[_policyId] = IOracleCommittee(_oracleCommittee);
    }
    ...
    function addNewProviderToCommittee(
        ...
        uint8 _decimals,
        bool _isOnChain
    ) public requireOracleCommitteeSet(_policyId) onlyOwnerOrOracleCommittee(_policyId) returns (address) {
    ...
    function getOracleCommitteeAddress(uint256 _policyId) public view returns (address) {
        return address(policyOracleCommittee[_policyId]);
    }

    struct OnlyTheMostRelevantPolicyMetadata {
        uint256 policyId;
        uint256 premiumPCT;
        uint256 blockNumber;
        address insuredTokenAddress;
        address collateralTokenAddress;
        address oracleCommitteeAddress;
        bool activated;
    }

    function getMetadataForPolicy(uint256 _policyId) public view returns (OnlyTheMostRelevantPolicyMetadata memory) {
        return OnlyTheMostRelevantPolicyMetadata({
            policyId: _policyId,
            premiumPCT: policyPremiumPCT[_policyId],
            blockNumber: policyBlock[_policyId],
            insuredTokenAddress: policyAsset[_policyId],
            collateralTokenAddress: policyCollateral[_policyId],
            oracleCommitteeAddress: address(policyOracleCommittee[_policyId]),
            activated: block.number >= policyBlock[_policyId] // This isn't accurate, but sufficient for demo
        });
    }
}
